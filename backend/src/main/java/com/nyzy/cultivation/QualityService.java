...
public class QualityService {
    ...
    public Long create(LandQuality e) {
        ...
        e.setDeleted(0);
        mapper.insert(e);
        auditLogService.record("quality", e.getId(), "CREATE", "新增质量评价 " + e.getParcelCode() + " " + e.getEvalYear() + "年");
        return e.getId();
    }
    ...
    public void update(LandQuality e) {
        ...
        e.setCreatedBy(null);
        e.setCreatedAt(null);
        if (mapper.updateById(e) == 0) {
            throw new ApiException(409, "数据已被他人修改, 请刷新后重试");
        }
        auditLogService.record("quality", e.getId(), "UPDATE", "修改质量评价");
    }
    ...
    public int batchUpdate(java.util.List<Long> ids, LandQuality u) {
        ...
        if (u.getGrade() != null && (u.getGrade() < 1 || u.getGrade() > 10)) throw new ApiException("地力等级必须为 1-10");
        boolean any = u.getEvalYear() != null || u.getGrade() != null || StringUtils.hasText(u.getSoilType())
                || StringUtils.hasText(u.getObstacle()) || StringUtils.hasText(u.getOrg());
        ...
        int n = 0;
        for (Long id : ids) {
            LandQuality old = mapper.selectById(id);
            if (old == null) continue;
            LandQuality patch = new LandQuality();
            patch.setId(id);
            patch.setVersion(old.getVersion());
            if (u.getEvalYear() != null) patch.setEvalYear(u.getEvalYear());
            if (u.getGrade() != null) patch.setGrade(u.getGrade());
            if (StringUtils.hasText(u.getSoilType())) patch.setSoilType(u.getSoilType());
            if (StringUtils.hasText(u.getObstacle())) patch.setObstacle(u.getObstacle());
            if (StringUtils.hasText(u.getOrg())) patch.setOrg(u.getOrg());
    ...
    public void delete(Long id, String reason) {
        ...
        auditLogService.record("quality", id, "DELETE", "软删除, 原因: " + reason);
    }
